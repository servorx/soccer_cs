...
public class MenuTransferencia
{
  ...
  public void DibujarMenu()
  {
    ...
    Console.WriteLine("========== MENÚ TRANSFERENCIA ==========\n");
    Console.ResetColor();
  ...
  private async Task ActualizarTransferenciaAsync()
  {
    ...
    Console.WriteLine($"Nuevo equipo origen (actual: {existente.EquipoOrigenId}), presiona enter para mantener el mismo nombre: ");
    ...
    if (!string.IsNullOrWhiteSpace(nuevoEquipoOrigen) && int.TryParse(nuevoEquipoOrigen, out int nuevoEquipoOrigenId))
      existente.EquipoOrigenId = nuevoEquipoOrigenId;

    Console.WriteLine($"Nuevo equipo destino (actual: {existente.EquipoDestinoId}), presiona enter para mantener el mismo nombre: ");
    ...
    if (!string.IsNullOrWhiteSpace(nuevoEquipoDestino) && int.TryParse(nuevoEquipoDestino, out int nuevoEquipoDestinoId))
      existente.EquipoDestinoId = nuevoEquipoDestinoId;
    ...
    Console.WriteLine($"Equipo origen: {existente.EquipoOrigenId}");
    Console.WriteLine($"Equipo destino: {existente.EquipoDestinoId}");
  ...
  private async Task VerTodoTransferenciaAsync()
  {
    ...
    foreach (var t in transferencias)
    {
      ...
      Console.WriteLine($"ID: {t.Id} | Tipo de transferencia: {t.TipoTransferencia} | Valor de transferencia: {t.ValorTransferencia:C} | Fecha de transferencia: {t.FechaTransferencia.ToShortDateString()} | Equipo origen: {t.EquipoOrigenId} | Equipo destino: {t.EquipoDestinoId}");
    }
  }
